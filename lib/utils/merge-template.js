...
module.exports = async function mergeTemplate(scope, rootPath) {
  ...
  const templatePath = await fse.mkdtemp(path.join(os.tmpdir(), 'siapi-'));
...
// Merge the template's template.json into the Siapi project's package.json
async function mergePackageJSON(rootPath, templateConfig, templateUrl) {
  ...
  if (templateConfig.package.siapi && templateConfig.package.siapi.uuid) {
    throw Error('A template cannot overwrite the Siapi UUID');
  }
  ...
  _.set(mergedConfig, 'siapi.template', templateUrl);
