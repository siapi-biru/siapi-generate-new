...
module.exports = (projectDirectory, cliArguments) => {
  ...
  const tmpPath = join(os.tmpdir(), `siapi${crypto.randomBytes(6).toString('hex')}`);
  ...
  const scope = {
    ...
    // use pacakge version as siapiVersion (all packages have the same version);
    siapiVersion: require('../package.json').version,
    ...
    siapiDependencies: [
      ...
      'siapi-admin',
      'siapi-utils',
      'siapi-plugin-content-type-builder',
      'siapi-plugin-content-manager',
      'siapi-plugin-users-permissions',
      'siapi-plugin-email',
      'siapi-plugin-upload',
      'siapi-plugin-i18n',
    ],
  ...
  sentry.configureScope(function(sentryScope) {
    const tags = {
      ...
      siapi_version: scope.siapiVersion,
