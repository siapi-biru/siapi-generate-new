...
module.exports = async function createProject(scope, { client, connection, dependencies }) {
  console.log(`Creating a new Siapi application at ${chalk.green(scope.rootPath)}.`);
  ...
  try {
    ...
    await fse.writeJSON(
      ...
      packageJSON({
        siapiDependencies: scope.siapiDependencies,
        additionalsDependencies: dependencies,
        siapiVersion: scope.siapiVersion,
        projectName: _.kebabCase(scope.name),
        uuid: scope.uuid,
        packageJsonSiapi: scope.packageJsonSiapi,
      }),
      {
        spaces: 2,
      }
  ...
  console.log('  Start Siapi in watch mode.');
  console.log();
  console.log(`  ${cmd} start`);
  console.log('  Start Siapi without watch mode.');
  console.log();
  console.log(`  ${cmd} build`);
  console.log('  Build Siapi admin panel.');
  console.log();
  console.log(`  ${cmd} siapi`);
